...
public class GameService implements Serializable {
     ...
     */
    public GameService() throws NoSuchAlgorithmException {
        generateRandomShipsAndPlaceThem();
    }

    public void generateRandomShipsAndPlaceThem() throws NoSuchAlgorithmException {
     ...
     */
    public JsonArray attackPlayerField( Coordinate missedCord) {
        ...
        serverTurnActions.add(Json.createObjectBuilder().add("missed",Json.createObjectBuilder().
                add("x",missedCord.getX()).add("y",missedCord.getY()).build()).build());
        ...
        while (serverTurn) {
            ...
            if (getPlayerField().getField()[coordToAttack.getX()][coordToAttack.getY()] == 1) {
                ...
                if (foundShip.getNumberOfDecks() == 0) {
                    JsonCreator.addInfoAboutDestroyedShip(serverTurnActions, foundShip);
                    continue;
                }
                JsonCreator.addInfoAboutDestroyedCord(serverTurnActions, coordToAttack);
            } else {
                serverTurn = false;
            }
        }
        ...
        if (arr.size() - 2 == 0) {
            return JsonCreator.returnInfoAboutCompleteMiss(missedCord);
        }
